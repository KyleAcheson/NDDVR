C AMMPOT4
C
C Library with routines for the definition of parameters and sections of the
C potential surface of ammonia and products.
C
C This program was adapted from a program written by
C          R. Marquardt
C          Laboratorium fuer Physikalische Chemie
C          ETH Zuerich (Zentrum)
C          8092 Zuerich - Switzerland
C
C Coordinates: R1, R2, R3, A12, A13, A23 (the 3 bond lengths and angles)
C
C Units are: angstroem for ri, and degrees for aij.
C 
C Evaluation of the PES at a given set of these coordinates is performed with
C
C    CALL VNH3(RINT,V)
C
C where RINT is a real*8 vector of length 6 containing the
C aforementioned coordinates in the given units. 
C
C The potential energy V is returned in the corresponding 
C units of a wavenumber (hc cm-1).
C
C Before any call to subroutine VNH3, subroutine VINI must be called once,
C to initiallize parameters.
C-------------------------------------------------------------------------------
      SUBROUTINE NH3POT(R, V, NP)
      IMPLICIT NONE
...
C version
      LPINT(NINT)='VERS. '
      PINT(NINT)=AMMPOT4
C assignment of model parameters
...
c conversion parameters to reciprocal centimeters
      PARAMETER(AJOULE=50341.12503,QJMOL=83.59346114)
c nominal parameters
c units are aJ=10**(-18)J for energies and A=100pm for lengths
      COMMON /PAR0/   V0(1:NI)
...
C map internal coordinates
      ...
      DO K=1,NI
         ...
         ENDDO
      ENDDO
C evaluate generic one dimensional switching functions
      DO K=1,NI
         DO J=1,N
...
c angular dependency of damping parameters
...
c
      DO K=1,NI
         DO J=1,N
            ...
            SPRE(J,K)=TANH(ARJ(J)*(R(J,K)-REJ(J)))
            SQRE(J,K)=1.-SPRE(J,K)
            SPCE(J,K)=TANH(ACJ(J)*(R(J,K)-REJ(J)))
            SQCE(J,K)=1.-SPCE(J,K)
         ENDDO
      ENDDO
...
c bending coordinates
      ...
      DO NO=1,NORD
         ...
         ENDDO
      ENDDO
C symmetrized anharmonic bending coordinates xy3 type molecules
      DO NO=1,NORD
         DO J=1,N
     ...
     &        +sb2b(J,4)**4
      ...
      DO NO=2,NORD
         ...
         ENDDO
      ENDDO
C calculate potential: initialization
      DO J=1,N
         VS(J) = 0.0
         VB(J) = 0.0   
         VP(J) = 0.0   
      ENDDO
C stretching potential
      DO K=1,NI
         DO J=1,N
...
C bending potential
      ...
      ENDDO
C asymptotic values for the pair potential
      KK=0
      DO K=1,NI
         DO L=(K+1),NI
            KK=KK+1
...
C pair potential
      DO KK=1,NNI
         ...
         ENDDO
      ENDDO
C substract DIII binding energy at the rate of increase of DJJ
C to avoid 3-fold clustering, substraction is inhibited whenever
C the distances of more than two YY bonds become too small
      KK=0
      DO K=1,NI
         DO L=(K+1),NI
            KK=KK+1
...
C definition of molecule specific potential parameters
      IMPLICIT REAL*8(A-H,O-Z)
